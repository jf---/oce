...
{
  statact = *this;
  statact.Description (sstat.nbph,sstat.otal,sstat.itle);
  sstat.oldph = sstat.phw = 0.;  sstat.numph = 0;
...
//  On cumule la phase precedente au total, on efface les donnees "locales"
  sstat.numcyc  = sstat.numst = sstat.olditp = 0;  sstat.oldst = sstat.stw = 0.;
  if (sstat.numph >= sstat.nbph) {  End();  return;  }

  sstat.numph ++;  sstat.oldph += sstat.phw;   // cumule sur cette phase
  sstat.nbitp = items;  sstat.nbcyc = cycles;
  statact.Phase(sstat.numph, sstat.n0,sstat.n1,sstat.phw,sstat.name);
  sstat.otph = (sstat.n1 > 1 ? statact.Step (sstat.n0) : 1.);
...
//  cumul de ce cycle sur les cycles deja passes, raz etapes
  sstat.numcyc ++;  sstat.olditp += sstat.nbitc;
//  if (sstat.olditem > sstat.nbitp) return;
  sstat.numst = 1;
  sstat.oldst = 0.;
  sstat.stw   = (sstat.n1 > 1 ? statact.Step(sstat.n0 + 1) : sstat.otph);
  sstat.nbitc = items;  sstat.numitem = 0;
}

    void  Interface_STAT::NextStep ()
{
  if (sstat.numst >= sstat.n1) return;
  sstat.numst ++;  sstat.oldst += sstat.stw;
  sstat.numitem = 0;
  sstat.stw = statact.Step (sstat.n0 + sstat.numst);
}

    void  Interface_STAT::NextItem (const Standard_Integer nbitems)
      {  sstat.numitem += nbitems;  }

    void  Interface_STAT::End ()
{  sstat.oldph = sstat.otal;  sstat.phw = sstat.stw = 0.;  sstat.itle = sstat.name = voidname;  }

// ###########  QUERY  ############

    Standard_CString  Interface_STAT::Where   (const Standard_Boolean phase)
      {  return (phase ? sstat.name : sstat.itle);  }

    Standard_Integer  Interface_STAT::Percent (const Standard_Boolean phase)
{
  if (sstat.numitem > sstat.nbitc) sstat.numitem = sstat.nbitc;
//  on compte les items deja passes
  Standard_Real enphase =
    sstat.olditp  * sstat.otph  +  // cycles complets passes
    sstat.nbitc   * sstat.oldst +  // cycle courant, etapes completes passees
    sstat.numitem * sstat.stw;     // etape courante
//  proportion pour cette phase
  Standard_Real prophase = enphase / (sstat.nbitp * sstat.otph);
  Standard_Integer res = Standard_Integer (prophase*100.);
  if (phase) return res;

//  voila pour cette phase
//  comptage dans les phases
  Standard_Real encours = (sstat.oldph + sstat.phw * prophase) / sstat.otal;
